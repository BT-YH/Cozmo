...
def alarm_clock(robot: cozmo.robot.Robot):
    ...
    while True:
        ...
        if do_alarm:
            ...
            with robot.perform_off_charger():
                short_time_string = str(current_time.hour % 12) + ":" + str(current_time.minute)
                robot.say_text("Wake up Barry you lazy human! it's " + short_time_string).wait_for_completed()
                robot.say_text("Come on Barry, wake up! it's " + short_time_string + ". You got a lot of work to do!").wait_for_completed()
                robot.say_text("Morning has broken! Like the first morning!" + 
                               "Blackbird has spoken! Like the first bird!" + 
                               "Praise for the singing! Praise for the morning!" + 
                               "Praise for them springing fresh from the Word!").wait_for_completed()
                robot.say_text("Come on Barry! A great day is waiting for you!!" +
                               "And a lot of work! Go out there and break a leg!").wait_for_completed()
        else:
